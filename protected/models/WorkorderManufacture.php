...
/**
 * This is the model class for table "workorder_manufacture".
 *
 * The followings are the available columns in table 'workorder_manufacture':
 * @property integer $id
...
class WorkorderManufacture extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('workorder_id, quantity, date', 'required'),
			array('workorder_id', 'numerical', 'integerOnly'=>true),
			array('quantity', 'numerical'),
			array('reference', 'length', 'max'=>100),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, reference, workorder_id, quantity, date', 'safe', 'on'=>'search'),
		);
	}
